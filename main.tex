...
We first collect a few useful technical lemmas in Section~\ref{technicalLemmas}.
Then we give the proof of Theorem~\ref{}. We develop a non-asymptotic theory for PCA in. HH.
...
%\end{proof}
\subsection{Technical Lemmas}\label{technicalLemmas}
...
\begin{lemma}[Davidson-Szarek bound]\label{matrixDeviation}
    Let $Z$ be an $N\times n$ ($N\geq n$) matrix whose entries are independent standard normal random variables.
    Then for every $t> 0$, with probability at least $1-2\exp (-t^2/2)$ one has
    $$
    \big(\max(\sqrt{N}-\sqrt{n}-t,0)\big)^2 \leq \lambda_{n}(ZZ^T)\leq \lambda_{1}(ZZ^T)\leq (\sqrt{N}+\sqrt{n}+t)^2.
    $$
    See~\cite{Davidson2001Local}.
\end{lemma}

\begin{lemma}\label{PCAlemma1}
Let $Y_1,\ldots, Y_n$ be i.i.d. standard normal random variables. Let $a_1,\ldots,a_n$ be nonnegative. We set 
$$
    |a|_{\infty}=\sup_{i=1,\ldots,n} |a_i|,\quad |a|_2^2=\sum_{i=1}^n a_i^2.
$$
    Then, for every $t>0$, with probability at least $1-\exp(-t^2/2)$, one has
    $$
    \sum_{i=1}^n a_i Y_i^2 \leq \sum_{i=1}^n a_i + \sqrt{2}|a|_2 t + |a|_{\infty} t^2.
    $$
    On the other hand, with probability at least $1-\exp(-t^2/2)$, one has
    $$
    \sum_{i=1}^n a_i Y_i^2 \geq \sum_{i=1}^n a_i - \sqrt{2}|a|_2 t.
    $$
    See~\cite{Laurent2000Adaptive}.
\end{lemma}
